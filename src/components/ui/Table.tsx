...
export function Table<T>({
  ...
  return (
    <div className={cn(CARD_SURFACE_CLASS, className)}>
            ...
            {data.length === 0 ? (
              <tr>
                <td
                  ...
                  className="px-4 py-12 text-center text-body-sm text-content-muted"
                >
                  {emptyMessage}
                </td>
              </tr>
              ...
              data.map((row, index) => {
                ...
                <tr
                  ...
                  className={cn(
                    ...
                    isSelected ? 'bg-brand-50/70 dark:bg-brand-600/10' : 'hover:bg-surface-muted/40',
                  )}
                >
                  {columns.map((column) => (
                    <td
                      key={column.key}
                      className={cn(
                        'whitespace-nowrap px-4 py-3.5 text-body-sm text-content-primary',
                        column.hideOnMobile && 'hidden md:table-cell',
                        column.className,
                      )}
                    >
